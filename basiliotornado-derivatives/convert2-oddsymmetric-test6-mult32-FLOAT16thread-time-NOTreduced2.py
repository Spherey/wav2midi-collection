...
import scipy.fft
from numpy import rot90, flipud, frombuffer, absolute, array, exp
import mido
import argparse
...
parser.add_argument("-o",
                    ...
                    default=0.96875) # y=-1/8x+1 where y is equal to the speed x i have no time to explain gtg to school
...
def spectrogram(data, samplerate, window, nperseg, overlap):
    ...
    for i in range(0, len(data), step):
        ...
        spec.append((fft / nperseg).astype('float16'))
...
def generate_spectrograms(note):
    ...
    dt, spectrogramL = spectrogram(dataL, samplerate, window=get_window("flattop", binSize), nperseg=binSize, overlap=overlap)
    __, spectrogramR = spectrogram(dataR, samplerate, window=get_window("flattop", binSize), nperseg=binSize, overlap=overlap)

    w = odd_symmetric(binSize)
